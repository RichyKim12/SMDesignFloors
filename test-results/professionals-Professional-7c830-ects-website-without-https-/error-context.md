...
- Name: professionals.spec.js >> Professionals Form — Validation >> rejects website without https://
- Location: tests\professionals.spec.js:147:3
...
Call log:
  - waiting for getByLabel(/website/i)
...
> 148 |     await page.getByLabel(/website/i).fill('www.badurl.com');
  ...
  242 |     await expect(page.getByRole('alert').first()).not.toBeVisible();
  243 |   });
  244 | 
  245 |   test('rejects a file with wrong extension disguised as PDF', async ({ page }) => {
  246 |     // A .exe file renamed to .pdf — magic bytes are MZ (not %PDF)
  247 |     const exeBytes = Buffer.from([0x4D, 0x5A, 0x90, 0x00, 0x03, 0x00, 0x00, 0x00]);
  248 |     await page.locator('input[type="file"]').first().setInputFiles({
```